...
H2 檔案 DB 位於 `mySpringAi_MCP_Server_stdio/h2db/mcpserver_stdio`，啟用 `AUTO_SERVER=true`，可以在 App 執行中同時用 H2 Console 或 DataGrip 連線。`ddl-auto=update`，適合本地開發，正式環境請改用 migration 流程。

首次啟動時 `DataInitializer` 會 seed 約 9 筆 `status=CLOSED` 的工單，當作 `troubleshootIssue`（sampling）的知識庫。
...
> 第一次要送 PR 前建議先讀過對應那份 `AGENTS.md` 中的 commit / 測試 / 格式規範。
