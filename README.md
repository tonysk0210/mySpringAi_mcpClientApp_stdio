...
- 🧠 **像資深客服一樣先幫你自助排障** —— 從歷史已結案工單庫中比對類似案例，回你一份具體的排除步驟建議（RAG + Sampling 能力）
- ❓ **執行到一半發現缺資料，暫停對話反問你** —— 「請問這件事的急迫程度？聯絡電話是？」——原本的建單流程**不會中斷、也不必重來**，使用者一補完資料，工具就從剛才停下的地方繼續往下跑（**Elicitation** 能力）
...
背後靠的是 **MCP（Model Context Protocol）** 搭配 Spring AI —— 讓 LLM 除了對話之外，也能透過標準化協定呼叫後端工具、存取資料庫、並在需要時與使用者互動。
